...
// Percorsi per salvare i dati e i report
...
function compareMembers(newMembers, oldMembers) {
  ...
  // Modifiche in isAdmin per i membri presenti in entrambi
  const adminChanges = [];
...
// Processa un singolo gruppo: fetch, confronto e salvataggio file
async function processGroup(group) {
  ...
  try {
    ...
    // Legge il file salvato in precedenza (se esiste)
    ...
    let reportContent = `Report per il gruppo "${group.name}" (${group.id})\n`;
    ...
    // Forza una modifica con timestamp per garantire che il contenuto cambi ad ogni esecuzione
    reportContent += `\nTimestamp: ${new Date().toISOString()}\n`;
    console.log(`Report per ${group.name}:`, reportContent);

    // Salva il report nella cartella REPORTS_DIR
    const reportFilePath = path.join(REPORTS_DIR, `${group.id}-report.txt`);
    await fs.writeFile(reportFilePath, reportContent, 'utf-8');
    console.log(`Report salvato in: ${reportFilePath}`);
